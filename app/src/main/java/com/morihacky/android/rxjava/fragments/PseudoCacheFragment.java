...
public class PseudoCacheFragment extends BaseFragment {
    ...
    /**
     * 延迟一秒再调用 {@link #getCachedDiskData()}
     *
     * @return
     */
        ...
        return Observable
                ...
                .doOnSubscribe(
                        (data) -> new Handler(Looper.getMainLooper()) //
                                .post(() -> adapterSubscriptionInfo.add("(磁盘) 正在缓存订阅数据"))) //
                .doOnComplete(
                        () -> new Handler(Looper.getMainLooper()) //
                                .post(() -> adapterSubscriptionInfo.add("(磁盘) 缓存完毕")));
    }
    ...
    private Observable<Contributor> getFreshNetworkData() {
        ...
        return githubService
                ...
                .doOnSubscribe(
                        (data) ->
                                new Handler(Looper.getMainLooper()) //
                                        .post(() -> adapterSubscriptionInfo.add("(网络) 正在缓存订阅数据"))) //
                .doOnComplete(
                        () ->
                                new Handler(Looper.getMainLooper()) //
                                        .post(() -> adapterSubscriptionInfo.add("(网络) 缓存完毕")));
    }
    ...
    private Map<String, Long> dummyDiskData() {
        ...
        map.put("JakeWharton - 磁盘", 0L);
        map.put("pforhan - 磁盘", 0L);
        map.put("edenman - 磁盘", 0L);
        map.put("swankjesse - 磁盘", 0L);
        map.put("bruceLee - 磁盘", 0L);
