...
public class ExponentialBackoffFragment extends BaseFragment {
    ...
    public void startRetryingWithExponentialBackoffStrategy() {
        ...
        Flowable.error(new RuntimeException("testing")) // always fails
                ...
                .doOnSubscribe(subscription -> _log("每 1s 请求一次，共请求 5 次"))
        ...
        Flowable
                ...
                .delay(
                        integer -> {
                            ...
                            return MathFlowable
                                    .sumInt(Flowable.range(1, integer))
                                    .flatMap(
                                            targetSecondDelay ->
                                                    Flowable
                                                            .just(integer)
                ...
                .doOnSubscribe(
                        s -> _log(
                                String.format("使用 delay 操作符执行 4 次任务 - time now: [xx:%02d]",
                                        _getSecondHand())))
                .subscribe(disposableSubscriber);
                    ...
                    new Function<Throwable, Publisher<?>>() {
                        ...
                        public Publisher<?> apply(Throwable throwable) {
                            if (++_retryCount < _maxRetries) {
                                ...
                                return Flowable.timer(_retryCount * _retryDelayMillis, TimeUnit
                                        .MILLISECONDS);
                            }
