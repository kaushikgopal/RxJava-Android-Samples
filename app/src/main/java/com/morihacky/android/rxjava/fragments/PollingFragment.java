...
public class PollingFragment extends BaseFragment {
    ...
    public void onStartSimplePollingClicked() {
        ...
        Disposable d =
                Flowable.interval(INITIAL_DELAY, POLLING_INTERVAL, TimeUnit.MILLISECONDS)
                        ...
                        .doOnSubscribe(
                                subscription -> {
                                    _log(String.format("开始简单轮询 - %s", _counter));
                                })
                        .subscribe(
                                taskName -> {
                                    _log(
    ...
    public void onStartIncreasinglyDelayedPolling() {
        ...
        _log(
                String.format(
                        Locale.US, "开始延迟逐渐递增的轮询，现在时间: [xx:%02d]",
                        _getSecondHand()));
    ...
    private class RepeatWithDelay implements Function<Flowable<Object>, Publisher<Long>> {
