    ...
    public void onListContributorsWithFullUserInfoClicked() {
      ...
      _subscriptions.add(
          _api.contributors(_username.getText().toString(), _repo.getText().toString())
              ...
              .observeOn(AndroidSchedulers.mainThread())
              .unsubscribeOn(Schedulers.newThread()) // TODO: remove when https://github.com/square/okhttp/issues/1592 is fixed
              .subscribe(new Observer<Pair<User, Contributor>>() {
                ...
                @Override public void onNext(Pair<User, Contributor> pair) {
                  ...
                  _adapter.add(
                      format("%s(%s) has made %d contributions to %s", user.name, user.email,
